...
browser.menus.onClicked.addListener(async function (info, tab){
  if(info.menuItemId == "findImg"){
    if (info.srcUrl){
      ...
      let bingSuffix = "https://www.bing.com/images/searchbyimage?FORM=IRSBIQ&cbir=sbi&imgurl=";

      let yandexTab = await browser.tabs.create({ 'active': false, 'url': yandexPrefix+imgUrl+yandexSuffix, 'index': tab.index+1 });
      let bingTab = await browser.tabs.create({ 'active': false, 'url': bingSuffix+imgUrl, 'index': tab.index+1 });
